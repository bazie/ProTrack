            ...
            $.each(fileInput.files, function (index, value) {
                if (value) {
                    if (size !== 0 && value.size > max_byte) {
                        errorBuilder({ [id]: `File ${value.name} Dépasse la limite maximale ${size / 1024} MB` });
                        fileInput.value = '';
                        return false;
                    }
                    ...
                    if (!check) {
                        errorBuilder({ [id]: `File ${value.name} Ne correspond pas aux types de fichiers autorisés` });
                        fileInput.value = '';
                        return false;
                    }
                }
