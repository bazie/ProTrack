{{--<script>--}}
    $(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status) {
                            ...
                            swal({
                                title: "Succès",
                                text: "Notification effacée avec succès",
                                type: "success",
        ...
        $('.logoutBtn').click(function (e) {
            ...
            swal({
                title: 'Etes vous sûr',
                text: "Vous serez déconnecté de cette application !",
                type: 'warning',
        ...
        $(document).on('click', '[data-toggle="control-sidebar"]', function () {
            ...
            if (page !== null) {
                $.ajax({
                    ...
                    dataType: 'json',
                    beforeSend: function () {
                        $('.control-sidebar .tab-content .media-list').html("<p class='text-center'>Chargement...</p>")
                    },
                    success: function (response) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.notifications.data.length > 0) {
                    ...
                    setTimeout(function () {
                        $('#notification-button').attr('title', `Bonjour, vous avez ${response.notifications.total} messages`).addClass('animated swing text-danger').tooltip('show');
                        hideTooltips('#notification-button')
                    }, 1000);

                } else {
                    head_notify.html(`<li class="text-center"><i class="fa fa-smile-o fa-2x text-primary"></i><a href="#" class="mt-0">Aucune notification non lue</a></li>`);
                    $('#notification-button').attr('title', `Bonjour, vous avez ${response.notifications.total} message`).removeClass('animated swing text-danger');
                }
            },
            error: function () {
                $('.notification-list').html("<p class='text-center'>Erreur de chargement des notifications</p>");
            }
        });
