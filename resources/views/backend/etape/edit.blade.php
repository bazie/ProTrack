{{ html()->form('POST', route($page->url . '.update-etape'))->id('form-create-' . $page->code)->acceptsFiles()->class('form form form-horizontal')->open() }}
<div class="panel shadow-sm">
    <div class="panel-body">
        <div class="row form-group">
            ...
            <div class='col-md-6'>
                ...
                {!! html()->text('nom_etape', $etape->nom_etape)->placeholder('Type Nom Etape here')->class('form-control')->id('nom_etape') !!}
            </div>
        </div>
        <div class="row form-group">
            <div class='col-md-6'>
                {!! html()->label()->class('control-label')->for('level_id')->text('Profil') !!}
                {!! html()->select('level_id', $levels->prepend('Sélectionner...', ''),$etape->level_id)->class('form-select')->id('level_id') !!}
            </div>
            <div class='col-md-6'>
                {!! html()->label()->class('control-label')->for('delai')->text('Delai') !!}
                {!! html()->number('delai', $etape->delai)->placeholder('Type Delai here')->class('form-control')->id('delai') !!}
            </div>
        </div>
                        ...
                        @foreach ($documents as $document)
                            <div class="col-md-3 document-{{ $document->id }}">
                                <input type="checkbox" name="documents_etape[]" value="{{ $document->id }}"
                                    id="document_{{ $document->id }}" class="filled-in chk-col-info" {{ $document->is_check }} />
        ...
        <div class="row">
            <table class="table table-bordered" id="metadata-table">
                ...
                <tbody>
                    @foreach ($metadatas as $metadonnee)
                        <tr>
                            <td><input type="hidden" name="metadatas[]" value="{{ $metadonnee->libelle }}"> {{ $metadonnee->libelle }}</td>
                            <td><input type="hidden" name="metadata_types[]" value="{{ $metadonnee->type_donnee }}">{{ $metadonnee->type_donnee }}</td>
                            <td><input type="hidden" name="is_requireds[]" value="{{ $metadonnee->obligatoire }}"> {{ $metadonnee->obligatoire ==1 ? 'Oui':'Non'}}</td>
    ...
    <script>
        $('.select2').select2();
        $('.modal-title').html(
            '<i class="fa fa-plus-circle"></i> Processus  {!! $processus->lib_processus !!} : Modifier etape ');
