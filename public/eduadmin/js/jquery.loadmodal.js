...
(function ($) {
    $.loadModal = function (options) {
        ...
        options.ajax.success.unshift(function (data, status, xhr) { // unshift puts this as the first success method
            ...
            let headTitle = options.title;
            let submitClass = headTitle.replace(/\s+/gi, '-').toLowerCase();
            let div = $([
                '<div id="' + options.id + '" class="modal ' + options.dlgClass + '" role="dialog" aria-labelledby="modalLabel' + options.bgClass + '"  aria-modal="false">',
                '  <div class="modal-dialog ' + options.size + '" role="document">',
                '      <div class="modal-content">',
                '        <div class="modal-header ' + options.bgClass + '" style="cursor: move;" title="click & geser modal">',
                '          <h4 class="modal-title" style="cursor: default;" id="modalLabel' + options.bgClass + '">' +
                ...
                '           <button class="btn btn-sm btn-secondary" data-bs-dismiss="modal"><i class="fa fa-ban"></i>Fermer</button>',
            ...
            div.on('hidden.bs.modal', function () {
                ...
                let modalBelow = $('.modal:visible').last();
                if (modalBelow.length) {
                    modalBelow.focus();
                    modalBelow.css('overflow-y', 'auto');
                }
            });
    ...
    $.showLoading = function () {
        $.blockUI({
            message: '<div class="loading"><i class="fa fa-refresh fa-spin"></i> Chargement...</div>',
            css: {
                'border': 'none',
                'padding': '15px',
                'backgroundColor': '#000',
